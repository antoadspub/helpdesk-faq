...
class WebsiteFaq(http.Controller):
    ...
    def faq_index(self, search='', category_id=None, **kwargs):
        ...
        if category_id:
            selected_category = FaqCategory.browse(category_id).exists()
    ...
    def faq_vote(self, question_id, helpful=True, **kwargs):
        ...
        # Public/anonymous endpoint with no login required - dedupe per
        # browser session so the same visitor can't repeatedly call this to
        # inflate/deflate the counters. Session-based, so it isn't proof
        # against a determined attacker clearing cookies, but stops the
        # trivial "click repeatedly" case.
        voted = request.session.setdefault('faq_voted_question_ids', [])
        if question_id in voted:
            return {
        ...
        request.session['faq_voted_question_ids'] = voted
        return {
            'helpful_count': question.helpful_count,
            'not_helpful_count': question.not_helpful_count,
        }
